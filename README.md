...
2. **The editor** — a browser-based editor for Hex Maniac Advance–modified
   ...
   fishing, rates, levels, species), and in-game NPC trades (received/requested
   species, nickname, held item, IVs, nature, OT info), with undo/redo, a diff
   view against a baseline ROM, reverse "Locations" lookup, and in-place saving
   with free-space repointing, backups, and write verification.
...
```sh
npm test                    # 98 tests incl. real-ROM integration (skipped if ROM absent)
